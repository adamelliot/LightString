...
#define TM1809_ALL_TABLE(_PORT,PTR, END, TABLE) \
while(PTR != END) { register unsigned char x = TABLE[*PTR * 3]; TM1809_BIT_ALL(_PORT); \
 x = TABLE[*PTR * 3 + 1]; TM1809_BIT_ALL(_PORT); \
	 x = TABLE[*PTR++ * 3 + 2]; TM1809_BIT_ALL(_PORT); }
...
else if(FastSPI_LED.m_eChip == CFastSPI_LED::SPI_TM1809)
{
  ...
  m_nDirty = 0;
  register byte *pData = m_pData;
  register byte *pPaletteTable = m_pPaletteTable;
  for(int iPins = 0; iPins < m_nPins; iPins++) { 
    register byte *pEnd = pData + m_pPinLengths[iPins];
    register unsigned char PIN = digitalPinToBitMask(FastSPI_LED.m_pPins[iPins]); 
    register volatile uint8_t *pPort = m_pPorts[iPins];
      ...
      if (m_usePaletteTable) {
        TM1809_ALL_TABLE(*pPort, pData, pEnd, pPaletteTable);
      } else {
        TM1809_ALL(*pPort, pData, pEnd); }
      }
  }
  sei();
}
