...
public:  
  ...
  // Custom extensions for LightString
  unsigned char m_bytesPerPixel;
  unsigned char *m_pPaletteTable;
  unsigned char m_paletteTableSize;
  unsigned char m_usePaletteTable;
public:
  ...
  void setLeds(int nLeds, unsigned char *pData) {
    m_bytesPerPixel = 3;
    m_nLeds = nLeds * m_bytesPerPixel;
    m_nCounter = 0;
    m_nDirty = 0;
    m_pData = pData;
 ...
 void setChipset(EChipSet);
  ...
  void setPixelData(unsigned char *pData) { m_pData = pData; m_pDataEnd = m_pData + m_nLeds; m_nDirty=1;}
  
  // call this method to get a pointer to the raw rgb data
  unsigned char *getRGBData() { return m_pData; }

  void setPaletteTable(unsigned char *paletteData) { m_usePaletteTable = 1; m_pPaletteTable = paletteData; m_nLeds /= m_bytesPerPixel; m_bytesPerPixel = 1; m_pDataEnd = m_pData + m_nLeds; }
  void cleanPaletteTable() { m_usePaletteTable = 0; m_pPaletteTable = 0; m_nLeds /= m_bytesPerPixel; m_nLeds *= (m_bytesPerPixel = 3); m_pDataEnd = m_pData + m_nLeds; }
