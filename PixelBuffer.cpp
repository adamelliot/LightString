...
PixelBuffer::PixelBuffer(CRGB *pixels, uint16_t length)
	: pixels(pixels), length(length)
{
	// LOG2("PIXELS: ", (uint16_t)pixels, length);
}

void PixelBuffer::setPixels(CRGB *pixels, uint16_t length) {
	this->pixels = pixels;
	this->length = length;
}

CRGB *PixelBuffer::getPixels() {
	return pixels;
}

uint16_t PixelBuffer::getLength() {
	return length;
}

void PixelBuffer::setSize(uint16_t width, uint16_t height) {
	this->width = width;
	this->height = height;
}

uint16_t PixelBuffer::getWidth() {
	return this->width;
}

uint16_t PixelBuffer::getHeight() {
	return this->height;
}

// TODO: Make this adjustable based on orientation
uint16_t PixelBuffer::getIndex(uint8_t x, uint8_t y) {
	return (height * x) + ((x % 2) == 1 ? y : (height - 1) - y);
}

// ============== Setting Colored Pixel Functions ==============

void PixelBuffer::setPixel(uint16_t index, CRGB col) {
	pixels[index] = col;
}

void PixelBuffer::setMirroredPixel(uint16_t index, CRGB col) {
	pixels[index] = col;
	pixels[length - index - 1] = col;
}

void PixelBuffer::setPixelAt(uint8_t x, uint8_t y, CRGB col) {
	pixels[getIndex(x, y)] = col;
}

// ============== Pixel Retrival Functions ==============

CRGB PixelBuffer::getPixel(uint16_t index) {
	return pixels[index];
}

// TODO: Fix this method
CRGB PixelBuffer::getPixelAt(uint8_t x, uint8_t y) {
	return CRGB::Black;
}

// ============== Full Buffer Modifiers ==============

void PixelBuffer::fade(float ratio) {
...
void PixelBuffer::clear(CRGB col) {
	for (int i = 0; i < length; i++)
		pixels[i] = col;
}

// Only shifts by 1 column
void PixelBuffer::shiftLeft() {
...
void PixelBuffer::shiftUp(uint16_t by) {
...
void PixelBuffer::shiftDown(uint16_t by) {
...
void PixelBuffer::drawLine(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, CRGB col) {
