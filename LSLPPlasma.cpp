...
double   grad(uint16_t hash, double x, double y, double z); 
//void     init_noise();
...
LSLightProgram *factoryPlasma(LSPixelBuffer *pixelBuffer, LSColorPalette* colorPalette, pcolor_func colorFunc) {
	return new LSLPPlasma(pixelBuffer, colorPalette, colorFunc);
}

LSLPPlasma::LSLPPlasma(LSPixelBuffer *pixelBuffer, LSColorPalette* colorPalette, pcolor_func colorFunc)
	: LSLightProgram(pixelBuffer, colorPalette, colorFunc)
{
	pixelBuffer->setSize(14, 14);

	colorIndex = random(0xff);
	changeRate = random(8) + 3;
}

void LSLPPlasma::update() {
/*	for (int x = 0; x < pixelBuffer->getWidth(); x++) {
		for (int y = 0; y < pixelBuffer->getHeight(); y++) {
	//Serial.println("Update");
	//delay(500);
			double val = sin((x + y * pixelBuffer->getWidth()) / (2.0 * M_PI));//pnoise(1.0, 1.0, 0.0);
			//Serial.println("pnoise done.");
			pixelBuffer->setPixel(x, y, colorPalette->getColor(val * 255.0));
		}
	}

	pixelBuffer->fade(0.1f);
*/
...
double pnoise(double x, double y, double z) 
{
	uint16_t	X = (uint16_t)floor(x) & 255,             /* FIND UNIT CUBE THAT */
		Y = (uint16_t)floor(y) & 255,             /* CONTAINS POINT.     */
		Z = (uint16_t)floor(z) & 255;
...
double grad(uint16_t hash, double x, double y, double z) 
{
uint16_t h = hash & 15;       /* CONVERT LO 4 BITS OF HASH CODE */
